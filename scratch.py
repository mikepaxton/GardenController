...
end_time_duration = [0] * len(watering_times)
schedule_running = [False] * len(relays)
...
def main():
    while True:  # Continue looping indefinitely
        ...
        for i, manual_button_state in enumerate(buttons):
            ...
            if not pause_schedule_button.value:
                time.sleep(.1)  # Introduce a small delay (0.1 seconds) for debounce and smoother button handling.
                ...
                if is_watering_day(i, current_day) and is_watering_time(i, current_time):
                    ...
                    if not manual_activation_flags[i] and end_time_duration[i] == 0:
                        ...
                        # Calculate the end time for the scheduled relay based off of the watering_times list
                        # in minutes plus the current monotonic time of the Pico.
                        end_time_duration[i] = watering_times[i][2] * 60 + int(time.monotonic())
                        schedule_running[i] = True  # Set the relay in "i" to running in schedule.

                else:
                    # Runs after a scheduled active relays duration ends.
                    relays[i].value = RELAY_INACTIVE
                    if debug: print(f"Relay {i + 1} for Garden Bed {i + 1} Off")
                    end_time_duration[i] = 0  # Reset the watering duration countdown when not watering.

                # Check if a watering schedule is currently running for the relay at index 'i' and if the
                # end time of the schedule has been reached based on the value stored in 'end_time_duration[i]'.
                if schedule_running[i] and end_time_duration[i] < int(time.monotonic()):
                    # Deactivate the relay by setting its value to RELAY_INACTIVE.
                    relays[i].value = RELAY_INACTIVE
                    # Update the schedule_running flag to indicate that the watering schedule has ended.
